...
- `npm run export-d1` —（選用）匯出 SQL 至 `d1/schema.sql`、`d1/data.sql`，若日後改回 D1 資料庫方案用。
- `npm start` — 本機 server `localhost:8080`（提供 `public/` + `/api/eta/:id` 代理 + 舊 DB API）。需先 build-db。
...
- `scripts/export-d1.js` —（選用）DB → SQL（D1 方案）。
- `server.js` — 本機開發 server：靜態托管 + ETA 代理 + 保留的 SQLite API（`/api/search`、`/api/stop/:id`、`/api/eta/:id`、`/api/stats`）。
...
- DB schema（`d1/schema.sql`）：`stops`、`routes`、`route_stops`、`stop_routes`。

## Conventions
- **前端**：Tailwind + Alpine.js + Phosphor Icons 全走 CDN；單頁一個 Alpine component 函數（`x-data`），頁尾內聯 `<script>`；相對路徑讀 `/api` 與 `data/*.json`。
...
- 產物（`bustop.db`、`data/`、`public/data/`、`node_modules/`、`.reasonix/`）皆 gitignore，勿提交。
