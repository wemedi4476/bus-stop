...
4. **收藏路線**：popup 內選「上班／回家」即加入（已在收藏中則切換分類）；寫入 localStorage 與主頁共享，並自動關閉 popup、捲回該站卡片。popup 內不提供移除。
5. **提示列**：顯示已收藏數量並指引到主頁。
...
- **寫入**：查詢頁選「上班／回家」加入或切換分類時寫入；主頁按站卡片的 ✕ 移除該站全部路線時寫入。
